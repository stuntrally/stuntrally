...
* 202 tracks (**34 New**)
    * 130 tracks renewed - added new Rocks, Particles, buildings, objects
    * New Stunt Rally **logo**, track TestC12-SR
* **7 New** vehicles, [topic](https://forum.freegamedev.net/viewtopic.php?f=80&t=18526)
    ...
    * 3B Scorpion - 3 balls futuristic racer, tricky, too wide, can't drive in pipes
    * Vehicle nick **names**, from animals [topic](https://forum.freegamedev.net/viewtopic.php?f=80&t=18531)
* Game
    ...
    * New 2nd level **main menu** "New game", with game modes and:
        * Game difficulty (new combo) - it changes:
            ...
            * also filters game modes lists (tutorials, championships, challenges)
    ...
    * Smoother play (apart from vegetation lags etc), higher game update frequency
    * Reworked Spaceships anti gravity, less ideal in normal simulation mode
* Graphics
    * New **rocks** (38 models), few new objects and 0AD buildings han, maur
    * Particles on tracks: **clouds**, fires, smoke, radioactive etc
    * Rivers, waterfalls WIP - done as Roads (toggle River alt-Enter and not looped N)
    * Road max visible distance, better Fps on big tracks
    * Better looking grass wind
* Editor
    * Sorted Pick lists, added separators and rating column (higher for best and popular)
        * All Road,Pipe,Wall,Column materials in 1 common list
    * Insert road point in middle of segment - Alt+Ins (**subdivide**)
    * *Many Roads* possible, see TestC11-Forks (but only first road is for checkpoints etc)
    * Not looped roads (N), in Start pos (Q)
        Enter toggles between start and end place
...
* GUI
    * Editor: tool windows with added icons, wider, split Help pages, etc
    * Vehicle tab
    ...
    * Track tab
        ...
        * preview and minimap images - click for fullscreen (or key Insert)
    ...
    * 3 new **tabs** in game Help
        ...
        * Credits, gathering all data txt files with links, authors and CC licenses
    * Moved
        * Separated SplitScreen tab. No Game tab, moved Boost etc to Setup subtab
        * Vehicle Colors now on 1st Setup subtab, added current color box
        * Multiplayer, split Server (unavailable) to last tab
* Other
    * New params in .car and fixes for new vehicles, bigger vehicles have *more grip*
    * [Conan](https://conan.io/downloads.html) build system added,  
        should be easier to build on Windows [PR here](https://github.com/stuntrally/stuntrally/pull/39)
    * sr-translator tool build in CMake, works also on Linux.  
        Added support for country language variations
    * Code cleanup, split big methods, files, use newer C++ auto, for, etc
* Removed old
    * Replays - Tools tab
    * 2 Super Racing cars, 5 VDrift cars and 4 tracks, VDrift sources reduced
* Known bugs
    ...
    * Editor somewhat broken, none or bad vegetation placing.
        * Due to bug in Ogre. It won't happen if using Ogre [13.5.2](https://github.com/OGRECave/ogre/releases/tag/v13.5.2) or higher.
...
* No usual binary releases made, git tag and new Flatpak build  
    available for GNU/Linux [here](https://flathub.org/apps/details/org.tuxfamily.StuntRally)
* **Fix** editor crashing on any Update (F8)
* Fixed silent sound for car type camera (e.g. in loops, car bonnet etc.)
...
* Sound
    * New **sound** engine in game, using [OpenAL Soft](https://openal-soft.org/)
    * [Reverberation](https://en.wikipedia.org/wiki/Reverberation) sound effect, changed with scenery
...
* New dependencies, if [compiling](compile.md) from sources
    * For sound get [libopenal-dev](https://pkgs.org/search/?q=libopenal-dev)
    * Unbundled bullet, get [libbullet-dev](https://pkgs.org/search/?q=libbullet-dev), and if present [libbullet-extras-dev](https://pkgs.org/search/?q=libbullet-extras-dev)
* Sources
...
- Common
    ...
    * Many smaller fixes
    * Changed translations, more info for strings, own program to generate .pot, [topic](https://forum.freegamedev.net/viewtopic.php?f=81&t=5729), [Localization](localization.md)
    * New user log files with errors and warnings only, ogre.err and ogre_ed.err
- Editor
    * [Easier on pipe](https://i.imgur.com/IGHsGQG.jpg) creating, mark (key 8) now also moves pipe down (ctrl-8 the old way)
...
* Simulation
    ...
    * **Sphere**, O in cars list, very bouncy
* Common
    * New **sky** textures, on half of tracks
    * New static objects on few tracks
    * Dropped SceneryID, each track has own cache
* Gui
    * [Car tab](https://i.imgur.com/DkuMS6p.jpg) with bars for stats, speed graph, short list view
    * Split, to new Graphics and Tweak tabs
    * Fonts bigger and resized with resolution
* Game
    * [Welcome screen](https://i.imgur.com/jaRlksI.jpg) with game Hints, shown on first run
    * Sounds for win, loose, lap, best time, wrong checkpoint
    * Fixed multiplayer nick appeared twice [topic](https://forum.freegamedev.net/viewtopic.php?f=78&t=5650#p57946)
    * Damage from terrain, height fog, fluids on few tracks
    * Possible to have different road surfaces on 1 track
* Editor
    * [Color pick window](https://forum.freegamedev.net/viewtopic.php?f=79&t=5581#p57754) with H,S,V sliders (for light,fog,trail colors)
...
* Tweak
    * Linear speed sensitive steering, [graph on Gui](http://imgur.com/1r4AH0Y)
...
* Fixed crash, happened in track loading, for tracks that have pipe transition segments.
...
* 104 tracks (5 new)
* Graphics
    * Using new **materials** generator: [shiny](https://forums.ogre3d.org/viewtopic.php?t=71117)
