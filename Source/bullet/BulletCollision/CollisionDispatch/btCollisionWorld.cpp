		...
		case SPHERE_SHAPE_PROXYTYPE:
			{
				...
				getDebugDrawer()->drawSphere(radius, worldTransform, color, 0);
				break;
			}
		case MULTI_SPHERE_SHAPE_PROXYTYPE:
			{
				...
				for (int i = multiSphereShape->getSphereCount()-1; i>=0;i--)
				{
					...
					getDebugDrawer()->drawSphere(multiSphereShape->getSphereRadius(i), worldTransform*childTransform, color, 0);
				}
		...
		case CAPSULE_SHAPE_PROXYTYPE:
			{
				...
				// Draw the ends
				{
					...
					getDebugDrawer()->drawSphere(radius, childTransform, color, -1);
				}{
					btTransform childTransform = worldTransform;
		...
		case CONE_SHAPE_PROXYTYPE:
			{
				...
				btVector3	ofsH(0,0,0);	ofsH[upAxis] = height * btScalar(0.5);
				btVector3	ofsR(0,0,0);	ofsR[(upAxis+1)%3] = radius;
				btVector3	ofsR2(0,0,0);	ofsR2[(upAxis+2)%3] = radius;

				const btMatrix3x3& wt = worldTransform.getBasis();
				btVector3   pos = start+wt * (ofsH);

				getDebugDrawer()->drawLine(pos,start+wt * (-ofsH+ofsR),color);
				getDebugDrawer()->drawLine(pos,start+wt * (-ofsH-ofsR),color);
				getDebugDrawer()->drawLine(pos,start+wt * (-ofsH+ofsR2),color);
				getDebugDrawer()->drawLine(pos,start+wt * (-ofsH-ofsR2),color);
				///+
				/*getDebugDrawer()->drawLine(start+wt * (-ofsH+ofsR),start+wt * (-ofsH+ofsR2),color);
				getDebugDrawer()->drawLine(start+wt * (-ofsH-ofsR),start+wt * (-ofsH-ofsR2),color);
				getDebugDrawer()->drawLine(start+wt * (-ofsH+ofsR),start+wt * (-ofsH-ofsR2),color);
				getDebugDrawer()->drawLine(start+wt * (-ofsH-ofsR),start+wt * (-ofsH+ofsR2),color);*/
				///o
				btVector3 x = wt * ofsR, y = wt * ofsR2;
				getDebugDrawer()->drawCircleZ(radius, start+wt *-ofsH, x,y, color);
				//getDebugDrawer()->drawCircleZ(radius, start+wt * ofsH, x,y, color);
				break;
			}
		...
		case CYLINDER_SHAPE_PROXYTYPE:
			{
				...
				btVector3	ofsH(0,0,0);	ofsH[upAxis] = halfHeight;
				btVector3	ofsR(0,0,0);	ofsR[(upAxis+1)%3] = radius;
				btVector3	ofsR2(0,0,0);	ofsR2[(upAxis+2)%3] = radius;

				const btMatrix3x3& wt = worldTransform.getBasis();
				getDebugDrawer()->drawLine(start+wt * (ofsH+ofsR),start+wt * (-ofsH+ofsR),color);
				getDebugDrawer()->drawLine(start+wt * (ofsH-ofsR),start+wt * (-ofsH-ofsR),color);
				getDebugDrawer()->drawLine(start+wt * (ofsH+ofsR2),start+wt * (-ofsH+ofsR2),color);
				getDebugDrawer()->drawLine(start+wt * (ofsH-ofsR2),start+wt * (-ofsH-ofsR2),color);
				///+
				/*getDebugDrawer()->drawLine(start+wt * ( ofsH+ofsR),start+wt * ( ofsH-ofsR),color);
				getDebugDrawer()->drawLine(start+wt * (-ofsH+ofsR),start+wt * (-ofsH-ofsR),color);
