...
App::App(SETTINGS *settings, GAME *game)
	...
	,valSSSEffect(0), valSSSVelFactor(0), valSteerRange(0)
	// rpl
	...
	,iTireSet(0), bchAbs(0),bchTcs(0), slSSSEff(0),slSSSVel(0), slSteerRng(0)
...
const String& App::GetGhostFile(std::string* ghCar)
{
	...
	while (srch)
	{
		...
		if (PATHMANAGER::FileExists(file))
		{	srch = false;  *ghCar = car;  }  // use same car. would have to recreate ghost car on lap!
		++i;
