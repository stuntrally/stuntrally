...
void ImpostorMaterialGenerator::generate(bool fixedFunction)
{
	...
	if (!vshader.isNull()) mgr.remove("Sprite_vp");
	...
	if (!fshader.isNull()) mgr.remove("Sprite_fp");
	...
	"	oColor = float4( hdrScale*lerp(texColor.xyz, fogColor.xyz, fog), texColor.a ); \n"
	"} \n";
	...
	fshader->getDefaultParameters()->setNamedConstant("hdrScale", mParent->getHDR() ? Real(mDef->mProps->hdrScale) : Real(1.0));
}
