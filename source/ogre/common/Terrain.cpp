...
void App::initBlendMaps(Ogre::Terrain* terrain)
{
	...
	for (x = 0; x < t; ++x)
	{
		...
		Real val1 = std::max(0.f, pow(0.5f + 0.5f*cosf(18.f* fy)*sinf(18.f* fx), p) - val);
		Real val2 = std::max(0.f, pow(0.5f + 0.5f*cosf(22.f* fy)*sinf(21.f* fx), q) - val-val1);
		Real val3 = std::max(0.f, pow(0.5f + 0.5f*cosf(19.f*fy)*sinf(20.f*fx), q) - val-val1-val2);
