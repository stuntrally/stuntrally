...
int ImpostorPage::impostorResolution = 128*2;  ///T par
...
void ImpostorTexture::renderTextures(bool force)
{
	...
	renderCamera->setNearClipDistance(/*nearDist*/0.1f); //+
	...
	if (needsRegen)
	{
		...
		{	
			...
			for (int i = 0; i < IMPOSTOR_YAW_ANGLES; ++i)
			{
				...
				renderTarget->update();
				if (o==0 && i==0)  ///+ removes junk on 1st render
				renderTarget->update();
			}
		}
