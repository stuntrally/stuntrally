...
void SoundBaseMgr::InitReverMap()
{
	...
	REV(PLAIN)
	REV(CARPETEDHALLWAY)  //RVB_CITY)  //RVB_PADDEDCELL)  //RVB_LIVINGROOM)  //none
	//RVB_ROOM)  //RVB_PLAIN)  //RVB_PARKINGLOT)  //none
	REV(SPORT_SQUASHCOURT)  // jungle

	REV(SEWERPIPE)  REV(UNDERWATER)  //+
	REV(STONECORRIDOR)  REV(HALLWAY)
	REV(DRIVING_TUNNEL)  REV(CITY_SUBWAY)
	REV(PIPE_SMALL)  REV(PIPE_LARGE)  REV(PIPE_LONGTHIN)  //pipe`

	REV(CASTLE_ALCOVE)  REV(CASTLE_COURTYARD)
	...
	REV(CASTLE_LONGPASSAGE)  REV(ICEPALACE_LONGPASSAGE)  REV(MOOD_HEAVEN)  REV(MOOD_HELL)  //crystals
